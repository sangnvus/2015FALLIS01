        ...
        private async void UpdateRiderHomeAddress(string inputAddress, string type)
        {
            ...
            try
            {
                ...
                if (myLocation.status.Equals(ConstantVariable.googleResponseStatusOK)) //OK
                {
                    ...
                    var input2 = string.Format("{{\"id\":\"{0}\",\"nCity\":\"{1}\",\"nAdd\":\"{2}\",\"lat\":\"{3}\",\"lng\":\"{4}\",\"addrType\":\"{5}\",\"olmd\":\"{6}\",\"role\":\"{7}\",\"cityId\":\"{8}\",\"cntry\":\"{9}\",\"uid\":\"{10}\",\"pw\":\"{11}\"}}", id, nCity, nAdd, lat.ToString().Replace(',', '.'), lng.ToString().Replace(',', '.'), addrType, lmd, role, cityId, cntry, uid, pw);
                    try
                    {
