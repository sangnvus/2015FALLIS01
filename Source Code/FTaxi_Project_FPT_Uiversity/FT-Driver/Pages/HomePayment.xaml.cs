...
namespace FT_Driver.Pages
{
    public partial class HomePayment : PhoneApplicationPage
    {
        ...
        private async void btn_Payment_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            ...
            string inputPassword = pw.FingerPrint;
            if (inputPassword.Equals(myPassword))
            {
                var input = string.Format("{{\"uid\":\"{0}\",\"pw\":\"{1}\",\"tid\":\"{2}\",\"eAdd\":\"{3}\",\"eCityName\":\"{4}\",\"eLat\":\"{5}\",\"eLng\":\"{6}\",\"dis\":\"{7}\",\"fare\":\"{8}\",\"lmd\":\"{9}\"}}", completeTrip.uid, inputPassword, completeTrip.tid, completeTrip.eAdd, completeTrip.eCityName, completeTrip.eLat, completeTrip.eLng, completeTrip.dis, completeTrip.fare, completeTrip.lmd);
                try
                {
